...
bool			Parser::parseFile(std::vector<std::shared_ptr<Road> >& roads, Graph &graph)
{
			...
			switch(i)
			{
				...
				else
				{
					...
					if (_points[pair] >= 2 && i != 7)
					{
						...
						// {
						// 	if (road->getPoints()[i].first == _lastNode.first && road->getPoints()[i].second == _lastNode.second)
						// 	{
						// 		Haversine haversine(road->getPoints()[i].first, road->getPoints()[i].second, _lastNode.first, _lastNode.second);
						// 		duration += (double) haversine.DistanceKm() / (double) road->getMaxSpeed() * 3600.0;
						// 	}
						// }
						// std::cout << "duration: " << duration << "s" << std::endl;

						int idx = getIndexOfLastNode(graph);

						graph.AddArc(idx, _nodeIndex - 1, duration);
						if (!road->getOneway())
						{
							graph.AddArc(_nodeIndex - 1, idx, duration);
						}
...
int				Parser::getIndexOfLastNode(Graph &graph)
{
	for (int i = 0; i < graph.getPositions().size(); ++i)
	{
		if (graph.getPositions()[i].first == _lastNode.first && graph.getPositions()[i].second == _lastNode.second)
		{
			return (i);
		}
	}
